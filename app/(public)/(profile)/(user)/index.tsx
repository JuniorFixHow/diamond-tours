          ...
          <View style={{width:'100%', flexDirection:'column',}} >
            ...
            <View style={{width:'100%', flexDirection:'row', justifyContent:'space-between', alignItems:'center'}} >
              <View style={{flexDirection:'column'}} >
                <Text style={{fontSize:20, fontWeight:'700'}}>{user?.fullName}</Text>
                <Text style={{fontSize:14, fontWeight:'300', color:'dimgrey'}}>{user?.emailAddresses[0]?.toString()}</Text>
              </View>
              <Image source={{uri:user?.imageUrl||profilephoto}} style={{width:80, height:80, borderRadius:40, objectFit:'cover'}} />
            </View>

            <View style={{width:'100%', flexDirection:'column', gap:5, marginTop:20,}} >
              <View style={styles.content} >
                <Text style={styles.label} >Email</Text>
                <Text style={styles.text} >{user?.emailAddresses[0]?.toString()}</Text>
              </View>
              {/* <View style={styles.content} >
